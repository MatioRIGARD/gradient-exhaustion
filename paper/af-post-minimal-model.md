...
A first production pass of the simulation then tested the pre-registered predictions: the **phase boundary sits exactly on the predicted line** $\delta = \eta c_h/\mu_h$ across the whole grid [Figure: f2_phase_diagram.png]; the **coexistence region under capability saturation is real** and tracks the analytic line $1 - \Lambda_{\max}/\bar\Lambda$ point by point [Figure: f2b_coexistence.png]; the collapse dynamics behave as derived [Figure: f1_pi_dynamics.png]; splitting the same aggregate capability across 1–20 operators leaves the exclusion time **exactly invariant** — confirming that without rent dissipation, competition among AIs protects no one [Figure: f4_operators.png]; and the demand-response experiment is the one described above [Figure: f3_hysteresis.png]. Two further pre-registered rounds probed the levers: **redistribution** (a tax on AI income transferred to humans) prevents exclusion only above a near-confiscatory threshold ($\tau^* = 1 - \delta\mu_h/(\eta c_h) \approx 0.92$ at baseline — the brake must beat exponential compounding); below it, transfers buy delay and survival but not participation, and after exclusion the human income share equals the tax rate exactly — the "kept alive, out of the loop" regime, now measurable. And **operator competition** protects humans only at near-total rent dissipation ($\rho^* \approx 0.97$), with the collusion case restoring the unprotected outcome exactly. A first pass at early-warning signals was negative (no detectable rise in lag-1 autocorrelation before collapse with a naive estimator) and is reported as such, not claimed. One registered prediction was refuted and is reported as such; the summary of verdicts lives next to the code.
