...
The **passive shareholder** is treated as "out of the loop": passive capital income is **kept out of π_market** and **reported as a separate series ρ** (not folded into Δ, headline indices unchanged). This is the choice consistent with a *participation* definition and with the rentier framing of the intelligence curse.

An objection we take seriously sharpened this into a two-channel accounting. The objection: what keeps a human "in the economic loop" is arguably the income-to-consumption link, not how the income is earned — a dividend spent is, demand-wise, indistinguishable from a wage spent. We agree, **for the demand channel**: in our model the demand side is deliberately source-agnostic (all human disposable income — earned, transferred, or capital — feeds demand identically). What π_market tracks is a different, supply-side question: whether the income flow is *self-enforcing*. Cutting a wage stops production; cutting a dividend stops nothing functionally — the claim survives only through institutional enforcement, whose alignment with humans is precisely the variable under study. So a dividend keeps a human economically *alive* (demand channel, measured), while π measures whether humans remain *needed* (participation channel, headline). If one held instead that owning AI capital is itself control — that a world where humans own the AI firms should score high — π as defined would understate human power; we keep the more demanding "owning is not participating" reading, state it as an explicit assumption, and let the ρ series test the alternative empirically rather than settle it by definition. **This two-channel accounting — demand source-agnostic, participation self-enforcing-only — is the choice I would most value an outside opinion on.**
